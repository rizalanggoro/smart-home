...
import com.bonfire.home.datas.BluetoothData;
...
results.add(result);
  ...
  public ArrayList<BluetoothData> getPairedDevices() {
    ArrayList<BluetoothData> hashMaps = new ArrayList<>();
    Set<BluetoothDevice> pairedDevices = bluetoothAdapter.getBondedDevices();

    if(pairedDevices.size() > 0) {
      for(BluetoothDevice device : pairedDevices) {
        BluetoothData a = new BluetoothData();
